  ...
  return function socketRateLimitMiddleware(socket, next) {
    ...
    socket.use(([event, ...args], nextFn) => {
      ...
      if (s.count > MAX_EVENTS) {
        ...
        if (s.warnings >= 3) {
          logger.warn('Socket disconnected — repeated rate limit abuse', { socketId: socket.id });
...
// ── Graceful shutdown ─────────────────────────────────────────────────────────
// Import activeTimers from gameSocket so we can clear them on shutdown
const { activeTimers } = require('./socket/gameSocket');
